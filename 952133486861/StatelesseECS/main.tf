terraform {
  ...
  backend "s3" {
    ...
    key            = "952133486861/StatelesseECS/main.tfstate"
    region         = "us-east-1"
    encrypt        = true
  }
}
...
data "aws_s3_bucket" "s3-off-load-wp-abcd1" {
...
data "aws_db_instance" "database4" {
  db_instance_identifier            = "database4"
}

data "aws_efs_file_system" "EFS2" {
  tags                              = {
    "Name" = "EFS2"
  }
}

data "aws_internet_gateway" "IGW3" {
  filter {
    name                            = "tag:Name"
    values                          = ["IGW3"]
  }
...
data "aws_security_group" "db_instance_database4_group" {
  filter {
    name                            = "tag:Name"
    values                          = ["db_instance_database4_group"]
  }
}

data "aws_security_group" "efs_file_system_EFS2_group" {
  filter {
    name                            = "tag:Name"
    values                          = ["efs_file_system_EFS2_group"]
  }
}
...
data "aws_iam_policy_document" "ecs_task_definition_WordPress_execution_st_StatelesseECS_doc" {
  statement {
    sid                             = "AllowWriteLogs"
    effect                          = "Allow"
  ...
  statement {
    ...
    resources                       = ["${data.aws_db_instance.database4.master_user_secret[0].secret_arn}"]
  }
}

resource "aws_iam_policy" "ecs_task_definition_WordPress_execution_st_StatelesseECS" {
  name                              = "ecs_task_definition_WordPress_execution_st_StatelesseECS"
  description                       = "Access Policy for WordPress (Role: execution)"
  policy                            = data.aws_iam_policy_document.ecs_task_definition_WordPress_execution_st_StatelesseECS_doc.json
}

data "aws_iam_policy_document" "ecs_task_definition_WordPress_task_st_StatelesseECS_doc" {
  statement {
    sid                             = "AllowEFSAccess"
    effect                          = "Allow"
    actions                         = ["elasticfilesystem:ClientMount", "elasticfilesystem:ClientRootAccess", "elasticfilesystem:ClientWrite"]
    resources                       = ["${data.aws_efs_file_system.EFS2.arn}:*"]
  }
  statement {
    sid                             = "AllowBucketLevelActions"
    effect                          = "Allow"
    actions                         = ["s3:GetBucketLocation", "s3:ListBucket"]
    resources                       = ["${data.aws_s3_bucket.s3-off-load-wp-abcd1.arn}"]
  }
  statement {
    sid                             = "AllowObjectCRUD"
    effect                          = "Allow"
...
resource "aws_iam_policy" "ecs_task_definition_WordPress_task_st_StatelesseECS" {
  name                              = "ecs_task_definition_WordPress_task_st_StatelesseECS"
  description                       = "Access Policy for WordPress (Role: task)"
  policy                            = data.aws_iam_policy_document.ecs_task_definition_WordPress_task_st_StatelesseECS_doc.json
}
...
resource "aws_iam_role_policy_attachment" "ecs_task_definition_WordPress_execution_st_StatelesseECS_attach" {
  policy_arn                        = aws_iam_policy.ecs_task_definition_WordPress_execution_st_StatelesseECS.arn
  role                              = aws_iam_role.execution_role_ecs_WordPress.name
}

resource "aws_iam_role_policy_attachment" "ecs_task_definition_WordPress_task_st_StatelesseECS_attach" {
  policy_arn                        = aws_iam_policy.ecs_task_definition_WordPress_task_st_StatelesseECS.arn
...
resource "aws_acm_certificate" "CertificateALB" {
  ...
  subject_alternative_names         = ["wp.cloudman.pro"]
...
resource "aws_subnet" "Public-b" {
  vpc_id                            = data.aws_vpc.WordPress3.id
...
resource "aws_route" "aws_route_RT5_IGW3" {
  gateway_id                        = data.aws_internet_gateway.IGW3.id
...
resource "aws_security_group_rule" "rule_SG_ECS_WP_to_db_instance_database4_group_tcp_3306" {
  security_group_id                 = data.aws_security_group.db_instance_database4_group.id
...
resource "aws_security_group_rule" "rule_SG_ECS_WP_to_efs_file_system_EFS2_group_tcp_2049" {
  security_group_id                 = data.aws_security_group.efs_file_system_EFS2_group.id
  source_security_group_id          = aws_security_group.SG_ECS_WP.id
...
resource "aws_cloudfront_distribution" "CDN2" {
  ...
  origin {
    domain_name                     = data.aws_s3_bucket.s3-off-load-wp-abcd1.bucket_regional_domain_name
...
resource "aws_efs_access_point" "AP2" {
  file_system_id                    = data.aws_efs_file_system.EFS2.id
  posix_user {
    gid                             = 33
    uid                             = 33
  }
...
locals {
  container_def_WordPress_Container1 = {
    ...
    "value" = "${data.aws_db_instance.database4.endpoint}"
  },
      {
    "name" = "AWS_DB_INSTANCE_DB_NAME_DB"
    "value" = "${data.aws_db_instance.database4.db_name}"
  },
      {
    ...
    "value" = "${data.aws_efs_file_system.EFS2.id}"
  },
      {
    ...
    "valueFrom" = "${one(data.aws_db_instance.database4.master_user_secret[*].secret_arn)}:password::"
  }
    ]
    "mountPoints" = [
      {
...
resource "aws_ecs_task_definition" "WordPress" {
  ...
  depends_on                        = [aws_iam_role_policy_attachment.ecs_task_definition_WordPress_execution_st_StatelesseECS_attach, aws_iam_role_policy_attachment.ecs_task_definition_WordPress_task_st_StatelesseECS_attach]
}
