...
data "archive_file" "archive_CloudManMain_Functionx-test" {
  output_path                       = "${path.module}/CloudManMain_Functionx-test.zip"
...
resource "aws_lambda_function" "Functionx-test" {
  ...
  architectures                     = ["arm64"]
  filename                          = "${data.archive_file.archive_CloudManMain_Functionx-test.output_path}"
  ...
  runtime                           = "python3.13"
  source_code_hash                  = "${data.archive_file.archive_CloudManMain_Functionx-test.output_base64sha256}"
  timeout                           = 2
  environment {
    variables                       = {
    "CICD_STAGE" = "test"
