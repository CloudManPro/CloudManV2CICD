terraform {
  ...
  backend "s3" {
    ...
    key            = "952133486861/AppCloudManV2/main.tfstate"
    region         = "us-east-1"
    encrypt        = true
  }
}
...
resource "aws_iam_policy" "lambda_function_DBAccessV2_st_AppCloudManV2" {
  name                              = "lambda_function_DBAccessV2_st_AppCloudManV2"
...
resource "aws_iam_policy" "lambda_function_HCLAWSV2_st_AppCloudManV2" {
  name                              = "lambda_function_HCLAWSV2_st_AppCloudManV2"
...
resource "aws_iam_role_policy_attachment" "lambda_function_HCLAWSV2_st_AppCloudManV2_attach" {
  policy_arn                        = aws_iam_policy.lambda_function_HCLAWSV2_st_AppCloudManV2.arn
  role                              = aws_iam_role.role_lambda_HCLAWSV2.name
}

resource "aws_acm_certificate" "AppCloudManV2" {
...
resource "aws_acm_certificate_validation" "Validation_AppCloudManV2" {
  certificate_arn                   = aws_acm_certificate.AppCloudManV2.arn
  validation_record_fqdns           = [for record in aws_route53_record.Route53_Record_AppCloudManV2 : record.fqdn]
}
...
resource "aws_route53_record" "Route53_Record_AppCloudManV2" {
  for_each                          = {for dvo in aws_acm_certificate.AppCloudManV2.domain_validation_options : dvo.domain_name => {
...
resource "aws_route53_record" "alias_a_dev_to_AppCloudManV2" {
  name                              = "dev.v2.cloudman.pro"
  zone_id                           = data.aws_route53_zone.Cloudman.zone_id
  type                              = "A"
  alias {
    name                            = aws_cloudfront_distribution.AppCloudManV2.domain_name
    zone_id                         = aws_cloudfront_distribution.AppCloudManV2.hosted_zone_id
    evaluate_target_health          = false
  }
}

resource "aws_route53_record" "alias_aaaa_dev_to_AppCloudManV2" {
  name                              = "dev.v2.cloudman.pro"
  zone_id                           = data.aws_route53_zone.Cloudman.zone_id
  type                              = "AAAA"
  alias {
...
resource "aws_api_gateway_deployment" "APIAppCloudManV2" {
  rest_api_id                       = aws_api_gateway_rest_api.APIAppCloudManV2.id
  lifecycle {
    create_before_destroy           = true
  }
  triggers                          = {
    "redeployment" = sha1(join(",", [jsonencode(aws_api_gateway_rest_api.APIAppCloudManV2.body)]))
  }
}
...
locals {
  ...
  openapi_spec_APIAppCloudManV2 = {
        openapi = "3.0.1"
        info = {
        title   = "APIAppCloudManV2"
        version = "1.0"
        }
        
        components = {
        securitySchemes = {
            "AppCloudManV2_CognitoAuth" = {
            type = "apiKey"
            name = "Authorization"
            in   = "header"
        ...
        paths = {
        for path in distinct([for i in local.api_config_APIAppCloudManV2 : i.path]) :
        path => merge([
...
resource "aws_api_gateway_rest_api" "APIAppCloudManV2" {
  name                              = "APIAppCloudManV2"
  body                              = jsonencode(local.openapi_spec_APIAppCloudManV2)
...
resource "aws_cloudfront_distribution" "AppCloudManV2" {
  ...
  default_cache_behavior {
    target_origin_id                = "origin_BucketApp"
  ...
  logging_config {
    bucket                          = aws_s3_bucket.app-cloudman-v2-logs.bucket_domain_name
  ...
  origin {
    ...
    origin_access_control_id        = aws_cloudfront_origin_access_control.oac_app-cloudman-v2.id
    origin_id                       = "origin_BucketApp"
  }
  origin {
    domain_name                     = "${aws_api_gateway_rest_api.APIAppCloudManV2.id}.execute-api.${data.aws_region.current.name}.amazonaws.com"
    origin_id                       = "origin_APIAppCloudManV2"
  ...
  viewer_certificate {
    acm_certificate_arn             = aws_acm_certificate.AppCloudManV2.arn
    cloudfront_default_certificate  = false
    minimum_protocol_version        = "TLSv1.2_2021"
    ssl_support_method              = "sni-only"
  }
  depends_on                        = [aws_s3_bucket_policy.aws_s3_bucket_policy_app-cloudman-v2-logs_st_AppCloudManV2]
}

resource "aws_cloudfront_origin_access_control" "oac_app-cloudman-v2" {
  name                              = "oac-app-cloudman-v2"
  description                       = "OAC for app-cloudman-v2"
...
resource "aws_s3_bucket" "app-cloudman-v2" {
  bucket                            = "app-cloudman-v2"
  force_destroy                     = true
  object_lock_enabled               = false
  tags                              = {
    "Name" = "app-cloudman-v2"
    "State" = "AppCloudManV2"
    "CloudmanUser" = "GlobalUserName"
  }
}

resource "aws_s3_bucket" "app-cloudman-v2-logs" {
  bucket                            = "app-cloudman-v2-logs"
  force_destroy                     = true
  object_lock_enabled               = false
  tags                              = {
    "Name" = "app-cloudman-v2-logs"
    "State" = "AppCloudManV2"
    "CloudmanUser" = "GlobalUserName"
  }
}

resource "aws_s3_bucket_acl" "app-cloudman-v2-logs_acl" {
  acl                               = "log-delivery-write"
  bucket                            = aws_s3_bucket.app-cloudman-v2-logs.id
  depends_on                        = [aws_s3_bucket_ownership_controls.app-cloudman-v2-logs_controls]
}

resource "aws_s3_bucket_ownership_controls" "app-cloudman-v2-logs_controls" {
  bucket                            = aws_s3_bucket.app-cloudman-v2-logs.id
  rule {
...
resource "aws_s3_bucket_ownership_controls" "app-cloudman-v2_controls" {
  bucket                            = aws_s3_bucket.app-cloudman-v2.id
  rule {
    object_ownership                = "BucketOwnerEnforced"
  }
}

data "aws_iam_policy_document" "aws_s3_bucket_policy_app-cloudman-v2-logs_st_AppCloudManV2_doc" {
  statement {
    sid                             = "AllowGetAcl"
    effect                          = "Allow"
    principals {
    ...
    resources                       = ["${aws_s3_bucket.app-cloudman-v2-logs.arn}"]
    condition {
      test                          = "StringEquals"
...
resource "aws_s3_bucket_policy" "aws_s3_bucket_policy_app-cloudman-v2-logs_st_AppCloudManV2" {
  bucket                            = aws_s3_bucket.app-cloudman-v2-logs.id
  policy                            = data.aws_iam_policy_document.aws_s3_bucket_policy_app-cloudman-v2-logs_st_AppCloudManV2_doc.json
}

data "aws_iam_policy_document" "aws_s3_bucket_policy_app-cloudman-v2_st_AppCloudManV2_doc" {
  statement {
    ...
    resources                       = ["${aws_s3_bucket.app-cloudman-v2.arn}/*"]
    condition {
      test     = "StringEquals"
      variable = "AWS:SourceArn"
      values   = ["arn:aws:cloudfront::${data.aws_caller_identity.current.account_id}:distribution/${aws_cloudfront_distribution.AppCloudManV2.id}"]
    }
  }
}

resource "aws_s3_bucket_policy" "aws_s3_bucket_policy_app-cloudman-v2_st_AppCloudManV2" {
  bucket                            = aws_s3_bucket.app-cloudman-v2.id
  policy                            = data.aws_iam_policy_document.aws_s3_bucket_policy_app-cloudman-v2_st_AppCloudManV2_doc.json
}

resource "aws_s3_bucket_public_access_block" "app-cloudman-v2-logs_block" {
...
resource "aws_s3_bucket_public_access_block" "app-cloudman-v2_block" {
  block_public_acls                 = true
  block_public_policy               = true
  bucket                            = aws_s3_bucket.app-cloudman-v2.id
  ignore_public_acls                = true
...
resource "aws_s3_bucket_server_side_encryption_configuration" "app-cloudman-v2-logs_configuration" {
  bucket                            = aws_s3_bucket.app-cloudman-v2-logs.id
...
resource "aws_s3_bucket_server_side_encryption_configuration" "app-cloudman-v2_configuration" {
  bucket                            = aws_s3_bucket.app-cloudman-v2.id
  expected_bucket_owner             = data.aws_caller_identity.current.account_id
  rule {
    bucket_key_enabled              = false
...
resource "aws_s3_bucket_versioning" "app-cloudman-v2-logs_versioning" {
  bucket                            = aws_s3_bucket.app-cloudman-v2-logs.id
...
resource "aws_s3_bucket_versioning" "app-cloudman-v2_versioning" {
  bucket                            = aws_s3_bucket.app-cloudman-v2.id
  versioning_configuration {
    mfa_delete                      = "Disabled"
    status                          = "Suspended"
